...
export function TreatmentPlansPage() {
  ...
  const createDietPlanPdfBlob = async (plan) => {
    ...
    const drawSectionTitle = (title) => {
      addPageIfNeeded(12);
      doc.setTextColor(8, 124, 104);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(13);
      doc.text(title, margin, y);
      y += 3;
      doc.setDrawColor(201, 221, 214);
    ...
    const drawMealHeader = () => {
      ...
      doc.setDrawColor(198, 219, 211);
      headers.forEach((header, index) => {
        doc.rect(x, y, widths[index], 9, 'FD');
        doc.setTextColor(22, 63, 51);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8.5);
        doc.text(header, x + 2, y + 5.7);
        x += widths[index];
      });
      y += 9;
    };
    ...
    doc.setTextColor(87, 118, 109);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text("Mom's Pathshala | Personalized Nutrition Plan", margin, y + 6);
    doc.setDrawColor(8, 124, 104);
    ...
    drawMealHeader();
    const meals = (plan.meals ?? []).length ? plan.meals : [{ time: '', meal: 'No meals added', food: '', notes: '' }];
    const widths = [22, 35, 75, 50];
    meals.forEach((meal) => {
      const values = [meal.time, meal.meal, meal.food, meal.notes].map((value) => clean(value, ''));
      const lines = values.map((value, index) => doc.splitTextToSize(value || '—', widths[index] - 4));
      const rowHeight = Math.max(9, Math.max(...lines.map((line) => line.length)) * 4 + 4);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    const instructionLines = doc.splitTextToSize(clean(plan.instructions, 'No additional instructions.'), contentWidth);
    addPageIfNeeded(instructionLines.length * 5 + 8);
    ...
    for (let page = 1; page <= totalPages; page += 1) {
      ...
      doc.setTextColor(117, 138, 130);
      doc.setFontSize(8);
      doc.text("Generated by Mom's Pathshala", margin, pageHeight - 6);
      doc.text(`Page ${page} of ${totalPages}`, pageWidth - margin, pageHeight - 6, { align: 'right' });
    }
    const blob = doc.output('blob');
    if (blob.size < 2500) throw new Error('Generated PDF is empty.');
    return blob;
  };
