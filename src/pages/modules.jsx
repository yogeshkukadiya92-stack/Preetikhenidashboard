...
function ClientProfile({ client, onBack }) {
  ...
  const savePayment = () => {
    ...
    setRefreshKey((k) => k + 1);
    setPayModal(false);
    setPayForm({ invoice: nextInvoiceNumber(updatedPayments), amount: '', paidAmount: '', pendingAmount: '', status: 'Paid', paymentMode: 'Cash', paidOn: new Date().toISOString().slice(0, 10) });
  };
    ...
    <section className="module-page">
      ...
      {activeTab === 'payments' && (
        <Card title="Payments" subtitle={`Payments linked to ${clientName}`}>
          <div className="table adaptive-table" style={{ '--table-columns': 7 }}>
            <div className="table-head">
              {['Invoice', 'Total', 'Paid', 'Pending', 'Status', 'Mode', 'Paid On'].map((h) => <div key={h}>{h}</div>)}
              <div />
            </div>
          ...
          <div className="modal-shell modal-small" role="dialog" aria-modal="true" aria-label="Add Payment" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body detail-grid">
              ...
              <label className="field-block">
                ...
                <select className="lead-input" value={payForm.paymentMode} onChange={(e) => setPayForm((f) => ({ ...f, paymentMode: e.target.value }))}>
...
export function PaymentsPage() {
  ...
  return (
    <ImportExportModule
      ...
      headers={['Client', 'Invoice', 'Total Amount', 'Paid Amount', 'Pending Amount', 'Status', 'Payment Mode', 'Paid On']}
      seedRows={payments}
      filenameBase="ayurflow-payments"
      fieldOptions={{ Client: clientNames, Status: ['Paid', 'Partial', 'Pending'], 'Payment Mode': ['Cash', 'GPay'] }}
      ...
      rowToValues={(row) => ({
        ...
        'Payment Mode': normalizePaymentRecord(row).paymentMode,
...
const financeTabs = [
  {
    ...
    columns: ['Client', 'Invoice', 'Total Amount', 'Paid Amount', 'Pending Amount', 'Status', 'Payment Mode', 'Paid On'],
...
export function TreatmentPlansPage() {
  ...
  const buildDietPlanPdfElement = (plan) => {
    const mealRows = (plan.meals ?? []).map((meal) => `<tr><td>${escapeHtml(meal.time)}</td><td>${escapeHtml(meal.meal)}</td><td>${escapeHtml(meal.food)}</td><td>${escapeHtml(meal.notes)}</td></tr>`).join('');
    const element = document.createElement('div');
    element.style.cssText = 'width:760px;padding:34px;background:#fff;color:#163f33;font-family:Arial,sans-serif;font-size:13px;line-height:1.45;';
    element.innerHTML = `<div style="border-bottom:3px solid #087c68;padding-bottom:14px;margin-bottom:18px;"><h1 style="margin:0;color:#087c68;font-size:26px;">Diet Plan</h1><p style="margin:4px 0 0;color:#57766d;">Mom's Pathshala · Personalized Nutrition Plan</p></div><div style="display:grid;grid-template-columns:repeat(2,1fr);gap:9px;margin-bottom:20px;"><div style="border:1px solid #d6e4df;border-radius:8px;padding:10px;"><b>Patient Name</b><br>${escapeHtml(plan.client)}</div><div style="border:1px solid #d6e4df;border-radius:8px;padding:10px;"><b>Service</b><br>${escapeHtml(plan.service)}</div><div style="border:1px solid #d6e4df;border-radius:8px;padding:10px;"><b>Goal</b><br>${escapeHtml(plan.goal)}</div><div style="border:1px solid #d6e4df;border-radius:8px;padding:10px;"><b>Duration</b><br>${escapeHtml(plan.duration)}</div><div style="border:1px solid #d6e4df;border-radius:8px;padding:10px;"><b>Calories</b><br>${escapeHtml(plan.calories)}</div><div style="border:1px solid #d6e4df;border-radius:8px;padding:10px;"><b>Water</b><br>${escapeHtml(plan.water)}</div></div><h2 style="margin:0 0 9px;font-size:18px;">Meal Schedule</h2><table style="width:100%;border-collapse:collapse;"><thead><tr><th style="background:#eaf5f1;border:1px solid #cbded7;padding:8px;text-align:left;">Time</th><th style="background:#eaf5f1;border:1px solid #cbded7;padding:8px;text-align:left;">Meal</th><th style="background:#eaf5f1;border:1px solid #cbded7;padding:8px;text-align:left;">Food</th><th style="background:#eaf5f1;border:1px solid #cbded7;padding:8px;text-align:left;">Notes</th></tr></thead><tbody>${mealRows || '<tr><td colspan="4" style="border:1px solid #d6e4df;padding:10px;">No meals added.</td></tr>'}</tbody></table><h2 style="margin:20px 0 7px;font-size:18px;">Instructions</h2><p style="white-space:pre-wrap;margin:0;color:#365e52;">${escapeHtml(plan.instructions)}</p><p style="margin:28px 0 0;padding-top:10px;border-top:1px solid #d6e4df;color:#779087;font-size:11px;">Generated by Mom's Pathshala</p>`;
    element.querySelectorAll('tbody td').forEach((cell) => {
      cell.style.cssText = 'border:1px solid #d6e4df;padding:8px;vertical-align:top;white-space:pre-wrap;';
    });
    return element;
  };
  ...
  const downloadDietPlanPdf = async (plan) => {
    setDietPdfMessage(`Creating PDF for ${plan.client || 'patient'}...`);
    try {
      const blob = await createDietPlanPdfBlob(plan);
      downloadBlob(`${fileSafeName(plan.client)}-diet-plan.pdf`, blob);
      setDietPdfMessage(`PDF downloaded for ${plan.client || 'patient'}.`);
    } catch {
...
export function ReportsPage() {
  ...
  const stats = (() => {
    ...
    if (activeReport === 'finance' && financeSubTab === 'payments') return [
      ...
      { label: 'Paid', value: paymentRows.filter((r) => r[5] === 'Paid').length },
      { label: 'Pending', value: paymentRows.filter((r) => r[5] === 'Pending').length },
